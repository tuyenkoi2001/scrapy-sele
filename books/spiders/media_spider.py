...
class MediaSpider(scrapy.Spider):
        ...
        for url in self.start_urls:
            try:
                ...
                for i in range(5):  # Cuộn 5 lần
                    ...
                    time.sleep(3)  # Chờ 3 giây mỗi lần cuộn
                # Ghi log số lượng hình ảnh tìm thấy
                images = self.driver.find_elements_by_tag_name('img')
                logging.info(f"Found {len(images)} images on {url} after scrolling")
                # Lấy nội dung trang
    ...
    def parse(self, response):
        # Lấy src, data-src, srcset từ thẻ img
        image_urls = (
            response.css('img::attr(src)').getall() +
            response.css('img::attr(data-src)').getall() +
            response.css('img::attr(srcset)').getall()
        )
        # Lọc và chuyển thành URL tuyệt đối
        absolute_image_urls = [urljoin(response.url, img_url.split(',')[0].split()[0]) for img_url in image_urls if img_url]
        self.logger.info(f"Extracted {len(absolute_image_urls)} image URLs: {absolute_image_urls}")
